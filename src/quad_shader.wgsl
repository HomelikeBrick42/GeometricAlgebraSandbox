...
fn vertex(input: VertexInput) -> VertexOutput {
    ...
    let uv = vec2<f32>(f32((input.vertex_index >> 0u) & 1u) - 0.5, f32((input.vertex_index >> 1u) & 1u) - 0.5);

    let quad = quads[input.quad_index];
    var world_position = uv * quad.size;
    world_position = vec2<f32>(world_position.x * cos(quad.rotation) - world_position.y * sin(quad.rotation), world_position.x * sin(quad.rotation) + world_position.y * cos(quad.rotation));
